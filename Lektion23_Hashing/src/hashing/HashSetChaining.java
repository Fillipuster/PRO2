...
public class HashSetChaining {
	...
	public boolean remove(Object x) {
		...
		while (bucket != null) {
			if (bucket.data.equals(x)) {
				if (prev == null) { // Did our removal match the first element?
					buckets[h] = bucket.next; // Set new first element to the previously second element;
				} else {
					prev.next = bucket.next; // Ordinary case;
				}
				return true;
			}
			prev = bucket;
